...
st.subheader("Understanding What Drives Predictions")
...
FEATURE_CATEGORIES = {
    ...
    'Movie Attributes': ['BUDGET', 'BUDGET_LOG', 'RUNTIME', 'POPULARITY', 'RELEASE_MONTH', 'IS_PEAK_SEASON'],
...
FEATURE_IMPORTANCE = {
    'YT_COMMENTS': 45.2, 'BUDGET': 38.7, 'BUDGET_LOG': 34.9, 'POPULARITY': 32.1,
    'ROLLING_7D': 28.5, 'AVG_STAR_POWER': 26.3, 'TOP2_STAR_POWER': 24.8, 'MAX_STAR_POWER': 22.1,
    'ROLLING_5D': 21.4, 'ROLLING_3D': 19.8, 'TRENDS_CUMULATIVE': 18.2, 'SENTIMENT': 16.9,
    'THEATRICAL_INTENT_PCT': 15.7, 'ENGAGEMENT_RATIO': 14.3, 'GENRE_ACTION_FRANCHISE': 13.8,
    'RELEASE_MONTH': 12.9, 'RUNTIME': 11.5, 'NET_INTENT_PCT': 10.8, 'VELOCITY_7D': 10.2,
    'IS_PEAK_SEASON': 9.8, 'IP_HIGH_PROFILE': 9.1, 'KNOWN_IP_TIER': 8.7, 'ROLLING_7D_PRIOR': 8.2,
    'TRENDS_PEAK_SO_FAR': 7.9, 'STAR_X_ROLLING': 7.5, 'VELOCITY_5D': 7.1, 'ROLLING_X_IP_HIGH': 6.8,
    'STREAMING_INTENT_PCT': 6.4, 'NUM_STARS_WITH_HISTORY': 6.1, 'GENRE_HORROR': 5.8,
    'SENTIMENT_X_ROLLING': 5.4, 'TRENDS_VOLATILITY': 5.1, 'VELOCITY_3D': 4.8,
    'ROLLING_5D_PRIOR': 4.5, 'ROLLING_3D_PRIOR': 4.2, 'GENRE_ANIMATION_FAMILY': 3.9,
    'PASS_INTENT_PCT': 3.6, 'INTENT_X_ROLLING': 3.3, 'IP_MODERATE': 3.0, 'DAYS_WITH_DATA': 2.8,
    'RATING_PG13': 2.5, 'RATING_R': 2.2, 'ROLLING_X_ACTION': 1.9, 'STAR_X_IP_HIGH': 1.7,
    'GENRE_PRESTIGE': 1.5, 'IP_NICHE': 1.3, 'RATING_PG': 1.1, 'GENRE_ORIGINAL': 0.9,
    'ROLLING_X_HORROR': 0.7, 'IP_ORIGINAL': 0.5, 'RATING_G': 0.3
}

st.divider()

st.header("Top 20 Features")
st.caption("By CatBoost importance score")
...
with col2:
    ...
    descriptions = {
        ...
        'Movie Attributes': "Budget, runtime, popularity score, release timing",
...
with col1:
    ...
    st.markdown("""
    1. **Google Trends** (118.8) - Real-time interest signals
    2. **Movie Attributes** (104.7) - Budget drives expectations
    3. **Star Power** (63.4) - Cast matters for draw
    4. **YouTube/Sentiment** (97.9) - Trailer reception + intent
    """)
...
with col2:
    ...
    - **Interactions** add marginal value (7.5% of total)
    """)
...
|--------------|----------|---------|---------|
| Static (Budget, Genre, Cast) | ✅ | ✅ | ✅ |
| Google Trends (Rolling) | ✅ | ✅ | ✅ |
| YouTube Engagement | ✅ | ✅ | ✅ |
| Intent Signals | ⚠️ Partial | ✅ | ✅ |

**Note**: All 51 features are available at all prediction horizons, but 
